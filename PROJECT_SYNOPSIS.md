...
**Improvements in Efficiency, Consistency, and Data Integrity:** The system improves efficiency by automating validation and report generation. Consistency is ensured through primary/foreign key constraints and transaction boundaries (COMMIT/ROLLBACK/SAVEPOINT). Data integrity is improved via normalization, constraints, and trigger-based audit trails.
...
- **User**: UserID, Name, Email, Role, PasswordHash, OperativeID, CreatedAt
- **Scan**: ScanID, UserID, FileHash, MediaType, Status, RiskScore, Confidence, VerdictID, ModelVersion, CreatedAt
- **AuditLog**: AuditID, UserID, Action, Resource, Timestamp
- **Notification**: NotificationID, UserID, Type, Message, IsRead, Timestamp
- **EvidenceVault**: EvidenceID, ScanID, FilePath, SHA256Hash, CreatedAt
- **VerdictType**: VerdictID, VerdictName, RiskMin, RiskMax

**Relationships:**

- User —creates—> Scan (1:N)
- User —has—> AuditLog (1:N)
- User —receives—> Notification (1:N)
- Scan —has—> EvidenceVault (1:0..1)
- Scan —references—> VerdictType (N:1)

**Cardinality & Constraints:**

- Each User can have many Scans; each Scan belongs to one User.
- Each Scan has at most one EvidenceVault record.
- Each Scan references exactly one VerdictType.
- AuditLog and Notification require a valid UserID.
...
erDiagram
    ...
    Scan ||--o| EvidenceVault : has
    Scan }o--|| VerdictType : references
```
